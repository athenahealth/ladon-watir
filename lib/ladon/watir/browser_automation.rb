...
module Ladon
  module Watir
    ...
    class BrowserAutomation < Ladon::Automator::ModelAutomation
      ...
      # Mapping of supported browser configuration flag names to a hash of metadata about those flags.
      #
      # The +setup+ method processes these flags in the order they appear (Ruby 1.9+),
      # defaulting to the metadata's +:default+ value. The :default may optionally be specified
      # as a proc taking the current +automation+ instance. This proc should return the desired default value.
      #
      # @return [Hash] Map of flag names to hash of metadata (default value, flag handler.)
      def self.browser_setup_flags
        {
            width: { default: FULL_SCREEN_SIZE,
                     handler: -> (automation, flag_value) { automation.browser_width = flag_value } },

            height: { default: FULL_SCREEN_SIZE,
                      handler: -> (automation, flag_value) { automation.browser_height = flag_value } },

            ui_url: { default: -> (automation) { automation.class.default_url },
                      handler: -> (automation, flag_value) { automation.browser.goto(flag_value) } }
        }
      end
      ...
      def build_browser
        browser_type = flags.get(BROWSER_FLAG, default_to: default_browser).to_sym
      ...
      # Processes the class' defined browser setup flags, configuring the browser appropriately.
      def setup
        self.class.browser_setup_flags.each do |flag, metadata|
          default_val = metadata[:default]

          # "default" can be a proc consuming this automation instance, returning the default value
      ...
      #
      # @param [Integer] width Should be the desired width of the browser, in number of pixels.
      #   May be the constant value +FULL_SCREEN_SIZE+ to indicate to maximize available width.
      def browser_width=(width)
        width = browser.execute_script('return screen.width;') if width == FULL_SCREEN_SIZE
        browser.window.resize_to(width.to_i, browser.window.size.height)
      end

      # Resize the browser's height to the given value.
      #
      # @param [Integer] height Should be the desired height of the browser, in number of pixels.
      #   May be the constant value +FULL_SCREEN_SIZE+ to indicate to maximize available height.
      def browser_height=(height)
        height = browser.execute_script('return screen.height;') if height == FULL_SCREEN_SIZE
